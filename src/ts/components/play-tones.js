...
var PlayTones = function (_a) {
    ...
    var keyboard = pianoKeys.map(function (key) {
        return <TouchableOpacity onPress={function () {
            return playNote(key.whiteKeyId);
        }} key={key.whiteKeyId} style={[styles.whiteKey, {height: Dimensions.get('window').height}]}>
            <View/>
        </TouchableOpacity>;
    });
    var keyboard2 = pianoKeys.map(function (key, i) {
        if (key.blackKeyId) {
            return <TouchableOpacity onPress={function () {
                return playNote(key.blackKeyId);
            }} activeOpacity={0.5} key={key.whiteKeyId} style={[styles.blackKey, {left: i * keyWidth - keyWidth / 4}]}>
                <View/>
            </TouchableOpacity>;
        }
    });
