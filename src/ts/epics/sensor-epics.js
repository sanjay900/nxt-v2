...
export function tickSensor(port, state$) {
    ...
    return merge(p.pipe(filter(function (sensor) { return sensor.type == SensorType.NONE; }), map(function () { return ({ rawValue: -1, scaledValue: -1, port: port }); })), p.pipe(filter(function (sensor) { return isUltrasonic(sensor.type) && sensor.enabled; }), switchMap(function () { return readI2CRegister(UltrasonicSensorRegister.MEASUREMENT_BYTE_0, port); }), map(function (data) {
